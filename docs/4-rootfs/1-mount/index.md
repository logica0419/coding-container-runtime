...
rootfs (ルートファイルシステム) と呼ばれる**ファイルシステム**を、`/` (ルートディレクトリ) に**マウント**することで、皆さんは`/`以下のディレクトリ・ファイルを使えるようになるのです。
...
コンテナで、**ホスト**のディレクトリ・ファイルを**コンテナ内でも**扱いたいとき「バインドマウント」をしますが、これがまさにその例です。
