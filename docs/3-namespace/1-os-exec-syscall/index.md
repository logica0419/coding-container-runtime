...
`bash`の子プロセスとして`make run`が実行され、`make run`の子プロセスとして`main` (今回作ったプログラム) が実行され、**その子プロセスとして**再び`bash`が実行されているという構図になっていますね。
...
プログラムを終了する際は、立ち上がった`bash`に以下のコマンドを打ってください。
