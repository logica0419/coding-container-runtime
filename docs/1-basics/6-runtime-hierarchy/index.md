...
- コンテナのルートディレクトリの中身を共通化して、**同じ環境を再現**したい
...
その**穴を埋めた**のがDockerです。  
Dockerが発表された当時、Dockerの中で実際にコンテナを作っていたのはLXCという**当時既存のコンテナランタイム**でした。コンテナの作成に対して新しいことはしていなかったのです。  
彼らが提供したのはコンテナを作るための**高レベルで便利なAPI**と**コンテナイメージ**。  
イメージの誕生によって、**同じ環境**を**軽量かつ大量**に起動するための最後のピースが埋まったのでした。
...
しかし、他社が次々とコンテナランタイムを出す中で、**コンテナイメージ**と**コンテナ作成機構**の**標準仕様**を策定する**OCI** (Open Container Initiative) という団体が作られました。  
**コンテナ作成機構**の標準化の過程で、libcontainerは切り出され**runc**として独立しました。
...
Dockerはこれを受け、CRIプラグインに必要な機能を抜き出して**containerd**として独立させ、それ以外の機能 (ネットワークドライバやイメージビルドなど) を**moby**として管理し始めたのです。
...
上記のような経緯で、現在のコンテナランタイムは**レイヤ分割**がされており、それぞれ以下のような名前と役割を持っています。
...
![Docker](./1.png){width=300}

![Podman](./2.png){width=300}
...
![containerd](./3.png){width=300}

![CRI-O](./4.png){width=300}
...
![youki](./5.png){width=200}
![crun](./6.png){width=200}
...
さあ、コンテナの実態とそのエコシステムについて、イメージは湧きましたか？  
いよいよここから**ハンズオン**に入っていきましょう！
