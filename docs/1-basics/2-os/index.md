...
皆さんご存知の通り、OSは**コンピューター全体の動作を管理・制御**し、コンピューター上で私たちが書いたプログラムが動くようにしてくれるソフトウェアです。  
...
Linux本体とディストリビューションの違いがわかっていなかった方は、ぜひこの機会に覚えておいてくださいね！
...
つまり、先ほどの「アプリケーションを実行するとプロセスになる」という説明は実は**正確ではなく**、**プロセスは器**であり、**器を増やしてアプリケーションを入れる**ことで作られる、という説明の方が正確です。
...
このプロセスの木構造は、`pstree`コマンドを使うことで確かめることができます。
