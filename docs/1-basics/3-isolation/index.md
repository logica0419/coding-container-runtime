...
chroot・pivot_rootは、どちらも (見かけ上の) **ルートディレクトリを変える**機能です。  
`/`**が別の場所にある**とOS側に認識させることで、その場所よりも**親側にある**ディレクトリ・ファイルは**閲覧・編集できなく**なります。  
[4章](/4-rootfs/)で触れますが、chrootとpivot_rootは**同じ機能を違う方法**で実現しています。

例えばこれらの機能を使い「`/home/container/tmp`がこれからは`/`だ」と設定すれば、設定されたプロセスは`/home/container/tmp`以下のファイルしか**閲覧できなく**なります。  
実行するプログラムや設定ファイル (`/etc`等) も**新しいルートを基準に探索**されるようになるため、適切なファイルを用意することで`/`以下のファイル群を**丸ごと入れ替えたように見せる**ことができます。

これも**プロセス単位**の概念で、あるプロセスの (見かけ上の) ルートディレクトリを変更すれば、その**子プロセスも**自動的に**そこが** (見かけ上の) **ルートディレクトリになり**ます。
...
これも**プロセス単位**の概念で、あるプロセスをあるcgroupに入れれば、基本的にはその**子プロセスもそのcgroupに所属**し、所属したプロセス**全体の合計量が制限**を受けます。
...
これらは今回のワークショップでは扱わないため、ぜひ各自で調べてみてください。
